...
# BIM (four parts total 3136 articles)
M_BIM_1 <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_BIM_part1.bib', dbsource = "wos", format = "bibtex")
M_BIM_2 <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_BIM_part2.bib', dbsource = "wos", format = "bibtex")
M_BIM_3 <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_BIM_part3.bib', dbsource = "wos", format = "bibtex")
M_BIM_4 <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_BIM_part4.bib', dbsource = "wos", format = "bibtex")
# combine_all M_BIM_1, M_BIM_2, M_BIM_3, M_BIM_4, using merge function
M_BIM <- merge(M_BIM_1, M_BIM_2, all = TRUE)
M_BIM <- merge(M_BIM, M_BIM_3, all = TRUE)
M_BIM <- merge(M_BIM, M_BIM_4, all = TRUE)
# print the dimension of the dataframe M_BIM
print(dim(M_BIM))

#print the min and max value in column "PY" in the dataframe M_BIM
print(min(M_BIM$PY))
print(max(M_BIM$PY))
print(sum(M_BIM$PY == 2025))
M_BIM$PY[M_BIM$PY == 2025] <- 2024
print(max(M_BIM$PY))


# ML
M_ML <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_ML.bib', dbsource = "wos", format = "bibtex")
print(dim(M_ML))
print(min(M_ML$PY))
print(max(M_ML$PY))
print(sum(M_ML$PY == 2025))
M_ML$PY[M_ML$PY == 2025] <- 2024
print(max(M_ML$PY))


# GIS
M_GIS <- convert2df(file = './Senstivity_Analysis/Data_Collection/SA_GIS.bib', dbsource = "wos", format = "bibtex")
print(dim(M_GIS))
print(min(M_GIS$PY))
print(max(M_GIS$PY))
print(sum(M_GIS$PY == 2025)) # we can see that there is 1 row has the value 2025 in column "PY" which need to be revised
# we have to classify the max year 2025 as 2024 in order to get the annual growth rate correctly
M_GIS$PY[M_GIS$PY == 2025] <- 2024
...
write.csv(M_final, file = './Senstivity_Analysis/overall_articles_selection_SA.csv', row.names = TRUE)
...
write.csv(annual_growth_rate, file = "./Senstivity_Analysis/annual_growth_rate_SA.csv", row.names = FALSE)

# ------plotting results for results 3.1----

# read the data from the csv file
final_dataset <- read.csv("./Senstivity_Analysis/overall_articles_selection_SA.csv")
...
df_growth_rates <- read.csv("./Senstivity_Analysis/annual_growth_rate_SA.csv")
df_growth_rates
...
p_figure_combined_1 <- ggarrange(p_a, p_b,p_h,p_f,p_c,p_g,p_e, p_d, ncol = 2, nrow = 4, labels = c("(a)", "(b)","(c)", "(d)","(e)", "(f)","(g)", "(h)") )
print(p_figure_combined_1)
ggsave("./Senstivity_Analysis/figureS2_senstivity_analysis.pdf", p_figure_combined_1, width = 10, height = 14, units = "in",bg = "white",dpi=300)
